...
2. **Check status to understand the schema**
   ...
   - `changeRoot` and `artifactPaths`: where the change lives and where its files are
...
3. **Get apply instructions**
   ...
   **Handle states:**
   - If `state: "blocked"` (missing artifacts): show the message and list `missingArtifacts`; suggest creating them first - for each missing artifact, run `openspec instructions <artifact-id> --change "<name>" --json` and write the file to its `resolvedOutputPath`
