...
2. **Get the change's artifacts**
   ...
   - `changeRoot` and `artifactPaths`: path context. Use these instead of assuming repo-local paths.
...
4. **Read and reconcile**
   ...
   - Revise only files that already exist (`existingOutputPaths`). Do NOT create artifacts that don't exist yet, and do NOT invent new files under a glob artifact - note them and tell the user they can be created as a follow-up (run `openspec instructions <artifact-id> --change "<name>" --json` to get the template and output path).
...
6. **Point to the next step (guidance only - NEVER act on it)**
   - Artifacts still missing -> suggest creating them next: for each missing artifact, `openspec instructions <artifact-id> --change "<name>" --json` returns the rules, template, and output path.
...
- Anything deferred as a follow-up (not-yet-created artifacts or files)
...
- Do not advance the build frontier: no new artifacts, no new files under glob artifacts - creating them is a separate follow-up step (via `openspec instructions <artifact-id>`), not part of an update.
